 ...
 * noContent.js
 *
 * A custom response when there is no content to return
 *
 * Example usage:
 * ```
 *     return res.noContent();
 *     // -or-
 *     return res.noContent(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function noContent(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.noContent()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.noContent()` called with an Error:', optionalData);
