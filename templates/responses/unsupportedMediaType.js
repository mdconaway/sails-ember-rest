 ...
 *
 * A custom response when the 'Content Type' is not accepted in a request
 *
 * Example usage:
 * ```
 *     return res.unsupportedMediaType();
 *     // -or-
 *     return res.unsupportedMediaType(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function unsupportedMediaType(optionalData) {
  ...
  var statusCodeToSet = 415;

  // If no data was provided, use res.sendStatus().
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.unsupportedMediaType()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.unsupportedMediaType()` called with an Error:', optionalData);
