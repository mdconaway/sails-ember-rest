...
module.exports = {
    ...
    prepareResourceAssociations(record, associations, primaryKey, json, toJSON, associatedRecords, include) {
        ...
        associations.forEach(association => {
            ...
            let assocModelIdentifier = pluralize(
                camelCase(sails.models[collection || model].globalId)
            );
            let assocModel;
            let assocType;
            let assocPK;
            ...
            if (type === 'collection') {
                ...
                if ((association.include === 'index' || association.include === 'record') && record[alias] && record[alias].length > 0) {
                    /* XXX
                ...
                if (association.include === 'link') {
                    links[alias] =
                        ...
                        '/' +
                        alias; //"/" + sails.config.blueprints.prefix
                    delete record[alias];
                }
    ...
    serializeResource(json, type, associations = [], linkSuffix = '') {
        const { id } = json;
        const attributes = Object.keys(omit(json, ['id'])).reduce((acc, key) => (
            Object.assign({}, acc, { [kebabCase(key)]: json[key] })
        ), {});
        const links = {
    ...
    generateResourceLink(modelPlural, linkSuffix = '') {
        ...
        return `${protocol}://${host}:${port}/${linkPrefix ? '/'+linkPrefix : ''}${modelPlural}${linkSuffix ? '/'+linkSuffix : ''}`;
    },
    ...
    buildResponse(model, records, associations, associatedRecords, include) {
        ...
        if (isCollection) {
            records.forEach(record => Ember.prepareResourceAssociations(record, associations, primaryKey, json, toJSON, associatedRecords, include));
        } else {
